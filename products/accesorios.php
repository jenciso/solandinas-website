...
<head>
	...
	<?$id = $_REQUEST["id"];
	$sqlstr="select * from saaccesorios where codigo=".$id;
	$filas = mysqli_query($conexion, $sqlstr);
	while ($fila = mysqli_fetch_array($filas)){?>
	<?$id=$fila['codigo'];?>
		<title><?echo$fila['name'];?> - Accesorios - Soldadoras Andinas</title>
	<? }?>
		...
		<section id="details">
			...
			<article class="center">
				...
				while ($fila = mysqli_fetch_array($filas)){?>
					<div id="wrapper" class="columns">
						<?$id=$fila['codigo'];?>
						<div class="img"><img src="../img/productos/accesorios/<?echo$fila['img'];?>.png"></div>
						<div class="text">
			...
			<article class="center">
				<div id="wrapper">
					<div class="regular slider">
						<?$id = $_REQUEST["id"];
						$sqlstr="select * from saaccesorios order by codigo asc";
						$filas = mysqli_query($conexion, $sqlstr);
						while ($fila = mysqli_fetch_array($filas)){?>
							<div class="prod">
								<?$id=$fila['codigo'];?>
								<a href="accesorios.php?id=<?echo$fila['codigo'];?>#details">
									<div class="content">
										<h3><?echo$fila['namereco'];?></h3>
									</div>
									<div class="img">
										<img src="../img/productos/accesorios/<?echo$fila['img'];?>.png">
										<o>leer más</o>
									</div>
								</a>
							</div>
