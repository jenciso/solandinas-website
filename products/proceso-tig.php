...
<head>
	...
	<?$id = $_REQUEST["id"];
	$sqlstr="select * from saprocesotig where codigo=".$id;
	$filas = mysqli_query($conexion, $sqlstr);
	while ($fila = mysqli_fetch_array($filas)){?>
	<?$id=$fila['codigo'];?>
		<title><?echo$fila['name'];?> - Proceso Tig - Soldadoras Andinas</title>
	<? }?>
			...
			<article class="center">
				...
				while ($fila = mysqli_fetch_array($filas)){?>
					<div id="wrapper">
					    <div class="columns">
    						<?$id=$fila['codigo'];?>
    						<div class="img"><img src="../img/productos/proceso-tig/<?echo$fila['img'];?>.png"></div>
    						<div class="text">
    							<h1><?echo$fila['name'];?></h1>
    							<?echo$fila['descrip'];?>
    							<div id="tablas"><img src="tablas/tig/<?echo$fila['modulos']?>.png" class="tablas"></div>
        					</div>
    				    </div>
					    <div class="center btn-fichatecnica"><a href="pdfs/tig/<?echo$fila['img'];?>.pdf" target="_blank" class="btn-ficha">ver ficha técnica</a></div>
					</div>
				<? }?>
			</article>
		</section>
			...
			<article class="center">
				<div id="wrapper">
					<div class="regular slider">
						<?$id = $_REQUEST["id"];
						$sqlstr="select * from saprocesotig order by codigo asc";
						$filas = mysqli_query($conexion, $sqlstr);
						while ($fila = mysqli_fetch_array($filas)){?>
							<div class="prod">
								<?$id=$fila['codigo'];?>
								<a href="proceso-tig.php?id=<?echo$fila['codigo'];?>#details">
									<div class="content">
