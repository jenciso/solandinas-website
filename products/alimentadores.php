...
<head>
	...
	<?$id = $_REQUEST["id"];
	$sqlstr="select * from saalimentadores where codigo=".$id;
	$filas = mysqli_query($conexion, $sqlstr);
	while ($fila = mysqli_fetch_array($filas)){?>
	<?$id=$fila['codigo'];?>
		<title><?echo$fila['name'];?> - Alimentadores - Soldadoras Andinas</title>
	<? }?>
			...
			<article class="center">
				...
				while ($fila = mysqli_fetch_array($filas)){?>
					<div id="wrapper">
					    <div class="columns">
    						<?$id=$fila['codigo'];?>
    						<div class="img"><img src="../img/productos/alimentadores/<?echo$fila['img'];?>.png"></div>
    						<div class="text">
    							<h1><?echo$fila['name'];?></h1>
    							<?echo$fila['descrip'];?>
    							<div id="tablas"><img src="tablas/alimentadores/<?echo$fila['modulos']?>.png" class="tablas tablas-cargadores"></div>
        					</div>
    				    </div>
					    <div class="center btn-fichatecnica"><a href="pdfs/alimentadores/<?echo$fila['img'];?>.pdf" target="_blank" class="btn-ficha">ver ficha técnica</a></div>
					</div>
				<? }?>
			</article>
		</section>
				...
				<div id="wrapper">
					<div class="regular slider">
						...
						while ($fila = mysqli_fetch_array($filas)){?>
							<div class="prod">
								<?$id=$fila['codigo'];?>
								<a href="alimentadores.php?id=<?echo$fila['codigo'];?>#details">
									<div class="content">
